-- Add videos for the Thai AI course lessons
WITH course_data AS (
...
(
  ...
  (SELECT id FROM lesson_data WHERE lesson_order = 2),
...
(
  ...
  (SELECT id FROM lesson_data WHERE lesson_order = 3),
  (SELECT course_id FROM course_data),
  NULL,
  now(),
  now()
...
(
  ...
  (SELECT id FROM lesson_data WHERE lesson_order = 4),
  (SELECT course_id FROM course_data),
  NULL,
  now(),
  now()
...
(
  ...
  (SELECT id FROM lesson_data WHERE lesson_order = 5),
  (SELECT course_id FROM course_data),
  NULL,
  now(),
  now()
...
video_lesson_mapping AS (
  ...
  JOIN public.lessons l ON v.lesson_id = l.id
  WHERE l.course_id = (SELECT course_id FROM course_data)
)
